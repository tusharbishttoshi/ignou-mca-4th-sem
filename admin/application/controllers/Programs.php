<?php
    class Programs extends Ci_controller{
        public function index(){
            ...
            $data['events'] = $this->mymodel->selcet_limit('*', 'programs', array('1'=>1), 'id', 30, 0);
            $this->load->view('main/program_list', $data);
        }
            ...
            if($this->form_validation->run()){
                if(empty($_FILES['image']['name'])){
                    ...
                    if($this->upload->do_upload('image')){
                        ...
                        $sql = $this->mymodel->insert('programs', $data);
                        if($sql){
                            $error['success'] = true;
                        }
        ...
        public function edit($id = 1){
            $data['event'] = $this->mymodel->select_one('*', array('id'=>$id), 'programs');
            if(!$data['event']){redirect(base_url('events'));}
            $this->load->view('main/program_edit', $data);
        }
        ...
        public function update(){
            ...
            if($this->form_validation->run()){
                ...
                $sql = $this->mymodel->update('programs', array('id'=>$formdata['edtid']), $data);
                if($sql){
                    $error['success'] = true;
                }else{
        ...
        private function getCong(){
            $config = array(
                'base_url' => base_url('programs/page'),
                'total_rows' => $this->mymodel->count_all('programs'),
                'per_page' => '30',
                'uri_segment' => 3,
